...
list "btpservice_cicd_credential_kubernetes_config" "<label_name>" {
...
# List block to discover all Kubernetes Configuration credentials in the SAP BTP CI/CD service.
# Returns only the resource identities by default.
list "btpservice_cicd_credential_kubernetes_config" "all" {
  provider = btpservice
}

# List block to discover all Kubernetes Configuration credentials with full resource details.
# Setting include_resource = true returns full resource objects (e.g., name, description).
list "btpservice_cicd_credential_kubernetes_config" "with_resource" {
  provider         = btpservice
  include_resource = true
}
