...
list "btpservice_cicd_credential_cloud_connector" "<label_name>" {
...
# List block to discover all Cloud Connector credentials in the SAP BTP CI/CD service.
# Returns only the resource identities by default.
list "btpservice_cicd_credential_cloud_connector" "all" {
  provider = btpservice
}

# List block to discover all Cloud Connector credentials with full resource details.
# Setting include_resource = true returns full resource objects (e.g., name, description).
list "btpservice_cicd_credential_cloud_connector" "with_resource" {
  provider         = btpservice
  include_resource = true
}
