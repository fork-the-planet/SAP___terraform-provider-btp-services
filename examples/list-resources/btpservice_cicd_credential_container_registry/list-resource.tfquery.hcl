...
list "btpservice_cicd_credential_container_registry" "<label_name>" {
...
# List block to discover all Container Registry credentials in the SAP BTP CI/CD service.
# Returns only the resource identities by default.
list "btpservice_cicd_credential_container_registry" "all" {
  provider = btpservice
}

# List block to discover all Container Registry credentials with full resource details.
# Setting include_resource = true returns full resource objects (e.g., name, description).
list "btpservice_cicd_credential_container_registry" "with_resource" {
  provider         = btpservice
  include_resource = true
}
