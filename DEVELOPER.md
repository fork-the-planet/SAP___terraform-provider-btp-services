...
If you want to contribute to the Terraform provider for SAP BTP Services, be aware of the [contribution guidelines](CONTRIBUTING.md) available in this repository.
...
[![Open in GitHub Codespaces](https://github.com/codespaces/badge.svg)](https://github.com/codespaces/new?hide_repo_select=true&ref=main&repo=terraform-provider-btp-services)

**Step 2:** There is no step 2 😎.

The following variant is supported for the GitHub Codespace:

* "Terraform provider for SAP BTP Services - Development" - the configuration in [.devcontainer/default/devcontainer.json](.devcontainer/default/devcontainer.json) contains the development override for the Terraform provider so that the local build is used. You must set the environment variables `BTP_CICD_ENDPOINT`, `BTP_CICD_TOKEN_URL`, `BTP_CICD_CLIENT_ID` and `BTP_CICD_CLIENT_SECRET` yourself.
...
```bash
git clone https://github.com/SAP/terraform-provider-btp-services.git
```
...
* "Terraform provider for SAP BTP Services - Development" - the configuration in [.devcontainer/default/devcontainer.json](.devcontainer/default/devcontainer.json) contains the development override for the Terraform provider so that the local build is used. You must set the environment variables `BTP_CICD_ENDPOINT`, `BTP_CICD_TOKEN_URL`, `BTP_CICD_CLIENT_ID` and `BTP_CICD_CLIENT_SECRET` yourself.
* Terraform provider for SAP BTP Services - Development (with env file) - the configuration in [.devcontainer/withenvfile/devcontainer.json](.devcontainer/withenvfile/devcontainer.json) enhances the "Terraform provider for SAP BTP Services - Development" to load environment variables when the container is started. This configuration expects a file called `devcontainer.env` in the folder `.devcontainer`.

> **Note**: `.env` files are excluded from git via `.gitignore`. You can use the file to store the environment variables `BTP_CICD_ENDPOINT`, `BTP_CICD_TOKEN_URL`, `BTP_CICD_CLIENT_ID` and `BTP_CICD_CLIENT_SECRET` that are needed when developing tests.
...
provider_installation {
  dev_overrides {
    "sap/btp-services" = "/path/to/go/bin" # the GOBIN directory can be found in the folder which `go env GOPATH` returns
  }
...
## Install the Terraform Provider for SAP BTP Services Locally
...
In case of errors, please check first that you executed the previous steps correctly. If you are still stuck, feel free to ask for support by raising a [question](https://github.com/SAP/terraform-provider-btp-services/discussions/categories/q-a) in the [GitHub Discussions](https://github.com/SAP/terraform-provider-btp-services/discussions/categories/q-a) of this repository.
...
* `feat(btpservice_cicd): a scoped feature`
